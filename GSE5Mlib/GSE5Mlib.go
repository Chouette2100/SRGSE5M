...
func InsertIntoOrUpdateUser(tnow time.Time, eventid string, roominf RoomInfo) (status int) {
	...
	if nrow == 0 {
		...
		err = Db.QueryRow(sql, userno).Scan(&name, &genre, &rank, &nrank, &level, &followers)
		if err != nil {
			log.Printf("err=[%s]\n", err.Error())
			status = -1
		}
...
func SelectRoomLevel(userno int, levelonly int) (roomlevelinf RoomLevelInf, status int) {
	...
	*/
	...
	for rows.Next() {
		Err = rows.Scan(&roomlevel.User_name, &roomlevel.Genre, &roomlevel.Rank, &roomlevel.Nrank, &roomlevel.Level, &roomlevel.Followers, &roomlevel.ts)
		if Err != nil {
